...
function abrirModalRemover() {
    document.getElementById("modalAdicionarMais").style.display = "none";
    document.getElementById("modalRemoverItem").style.display = "flex";
}

function fecharModalRemover() {
    document.getElementById("modalRemoverItem").style.display = "none";
    document.getElementById("modalAdicionarMais").style.display = "flex";
}

function removerItem() {
    let nomeItem = document.getElementById("modalInputRemover").value.trim().toLowerCase();
    ...
    for (const departamento in listaDeCompras) {
        let indice = listaDeCompras[departamento].findIndex(item => item.toLowerCase() === nomeItem);

        if (indice !== -1) {
            listaDeCompras[departamento].splice(indice, 1);
            itemRemovido = true;

            if (listaDeCompras[departamento].length === 0) {
                delete listaDeCompras[departamento];
            }
            break;
        }
    }
    ...
    if (itemRemovido) {
        let nomeFormatado = nomeItem.charAt(0).toUpperCase() + nomeItem.slice(1);
        alert(`${nomeFormatado} foi removido da lista.`);
        atualizarLista();
        fecharModalRemover();
    } else {
